...
#         st.image(image, use_column_width=False)
#     else:
#         st.warning("Poster link not found for the movie.")
def movie_poster_fetcher(imdb_link):
...
def get_movie_info(imdb_link):
    ...
    if imdb_content is not None and 'content' in imdb_content.attrs:
        ...
        # Ensure that there are enough elements in the list
        if len(movie_descr) >= 3:
            movie_director = movie_descr[0]
            movie_cast = str(movie_descr[1]).replace('With', 'Cast: ').strip() if len(movie_descr) > 1 else 'Cast Not Found'
            movie_story = 'Story: ' + str(movie_descr[2]).strip() + '.' if len(movie_descr) > 2 else 'Story Not Found'

            # Use a more specific class for rating (modify as needed)
            rating = s_data.find("span", {"class": "rating"})
            movie_rating = 'Total Rating count: ' + str(rating.text) if rating else 'Rating Not Found'

            return movie_director, movie_cast, movie_story, movie_rating
        # else:
        #     return "Director Not Found", "Cast Not Found", "Story Not Found", "Rating Not Found"
    else:
        return "Description Not Found", "", "", "Rating Not Found"
